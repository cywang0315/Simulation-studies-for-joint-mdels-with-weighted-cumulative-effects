...
cum_cubicbs=function(t0,tt){ # tt can be a vector
  if(any(t0>tt)){
    ...
    cum_cubicbs_sing=function(k,l,t){# t is a scalar, k is a scalar
      if(l==1){
        s0=ifelse((knots[k+1]<t0)|(knots[k]>t),0,(min(t,knots[k+1])-max(t0,knots[k])))
        s1=ifelse((knots[k+1]<t0)|(knots[k]>t),0,1/2*(min(t,knots[k+1])^2-max(t0,knots[k])^2))
        s2=ifelse((knots[k+1]<t0)|(knots[k]>t),0,1/3*(min(t,knots[k+1])^3-max(t0,knots[k])^3))
        s3=ifelse((knots[k+1]<t0)|(knots[k]>t),0,1/4*(min(t,knots[k+1])^4-max(t0,knots[k])^4))
        return(list(s0=s0,s1=s1,s2=s2,s3=s3))
      }else{
        if(l==2){
          s01=ifelse((knots[k+l-1]<t0)|(knots[k]>t),0,
                    1/(knots[k+l-1]-knots[k])*cum_cubicbs_sing(k,l-1,t)$s1-knots[k]/(knots[k+l-1]-knots[k])*cum_cubicbs_sing(k,l-1,t)$s0)
          s02=ifelse((knots[k+l]<t0)|(knots[k+1]>t),0,
                 knots[k+l]/(knots[k+l]-knots[k+1])*cum_cubicbs_sing(k+1,l-1,t)$s0-1/(knots[k+l]-knots[k+1])*cum_cubicbs_sing(k+1,l-1,t)$s1)
          s0=s01+s02
          s11=ifelse((knots[k+l-1]<t0)|(knots[k]>t),0,
                     1/(knots[k+l-1]-knots[k])*cum_cubicbs_sing(k,l-1,t)$s2-knots[k]/(knots[k+l-1]-knots[k])*cum_cubicbs_sing(k,l-1,t)$s1)
          s12=ifelse((knots[k+l]<t0)|(knots[k+1]>t),0,
                     knots[k+l]/(knots[k+l]-knots[k+1])*cum_cubicbs_sing(k+1,l-1,t)$s1-1/(knots[k+l]-knots[k+1])*cum_cubicbs_sing(k+1,l-1,t)$s2)
          s1=s11+s12
          s21=ifelse((knots[k+l-1]<t0)|(knots[k]>t),0,
                     1/(knots[k+l-1]-knots[k])*cum_cubicbs_sing(k,l-1,t)$s3-knots[k]/(knots[k+l-1]-knots[k])*cum_cubicbs_sing(k,l-1,t)$s2)
          s22=ifelse((knots[k+l]<t0)|(knots[k+1]>t),0,
                     knots[k+l]/(knots[k+l]-knots[k+1])*cum_cubicbs_sing(k+1,l-1,t)$s2-1/(knots[k+l]-knots[k+1])*cum_cubicbs_sing(k+1,l-1,t)$s3)
          s2=s21+s22
    ...
    cum_cubicbs_t=function(t){
      return(sapply(1:(length(internal_knots)+3+1),cum_cubicbs_sing,l=4,t=t))
    }
    
    return(t(sapply(tt,cum_cubicbs_t)))
  }
...
inival=function(data,data.id,ctl,knots,Q.2,low,up){
  ...
  conloglik_3=function(par){
    ...
    for(i in data.id$id[data.id$delta==1]){
      ...
      conloglik=conloglik+gamma*data.id$W[i]+alpha1*sum(c(cum_cubicbs(1,Ti))*(beta+t(random.effects(lme.data)[i,])))+
        alpha2*sqrt(sum((beta+t(random.effects(lme.data)[i,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[i,])))))-
        log(sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cum_cubicbs(1,Ti))*(beta+t(random.effects(lme.data)[rs.id,])))+
                      alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))))
    }
    return(-conloglik)
  ...
  gg_conloglik_3=function(par){
    ...
    for(i in data.id$id[data.id$delta==1]){
      Ti=data.id$Time[data.id$id==i]
      rs.id=data.id$id[data.id$Time>=Ti]
      s=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cum_cubicbs(1,Ti))*(beta+t(random.effects(lme.data)[rs.id,])))+
                  alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,])))))))
      s1=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cum_cubicbs(1,Ti))*(beta+t(random.effects(lme.data)[rs.id,])))+
                   alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*data.id$W[rs.id])
      
      s2=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cum_cubicbs(1,Ti))*(beta+t(random.effects(lme.data)[rs.id,])))+
                   alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*colSums(c(cum_cubicbs(1,Ti))*(beta+t(random.effects(lme.data)[rs.id,]))))
      
      i2=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cum_cubicbs(1,Ti))*(beta+t(random.effects(lme.data)[rs.id,])))+
                   alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*
               (colSums(c(cum_cubicbs(1,Ti))*(beta+t(random.effects(lme.data)[rs.id,]))))^2)
      
      s3=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cum_cubicbs(1,Ti))*(beta+t(random.effects(lme.data)[rs.id,])))+
                   alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))
      
      i3=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cum_cubicbs(1,Ti))*(beta+t(random.effects(lme.data)[rs.id,])))+
                   alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*
               (colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))
      gg_gamma=gg_gamma+data.id$W[i]-s1/s
      gg_alpha1=gg_alpha1+sum(c(cum_cubicbs(1,Ti))*(beta+t(random.effects(lme.data)[i,])))-s2/s
      i_alpha1=i_alpha1+i2/s-(s2/s)^2
      gg_alpha2=gg_alpha2+sqrt(sum((beta+t(random.effects(lme.data)[i,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[i,])))))-s3/s
  ...
  alpha1=0
  alpha2=0
  sig2_set=data.frame(sig2=c(0.1,2,5,10))

  for(k in 1:dim(sig2_set)[1]){
    sig2=sig2_set$sig2[k]
    res=optim(c(gamma,alpha1,alpha2),conloglik_3,gg_conloglik_3,method = "BFGS")
    sig2_set$value[k]=res$value
    sig2_set$gamma[k]=res$par[1]
    sig2_set$alpha1[k]=res$par[2]
    sig2_set$alpha2[k]=res$par[3]
  }
  
  sig2=sig2_set[which.min(sig2_set$value),]$sig2
  ...
  #alpha2=sig12_set[which.min(sig12_set$value),]$alpha2
   ...
   for(k in 1:10){
    ...
    alpha1=res$par[2]
    alpha2=res$par[3]
    sig2_res=optim(c(sig2),conloglik_sig2,lower=c(0.01),upper = c(15),method = "L-BFGS-B")
    sig2=sig2_res$par
    if(res$value<sig2_res$value){
      break
    }else{
      if(abs(res$value-sig2_res$value)/res$value<10^(-6)){
        break
      }
    }
  }

  coxts=c(gamma,alpha1,alpha2,sig2)
  ...
  for(t in sort(unique(data$Time[data$delta==1]))){
    rs.id=riskset(t)
    s=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cum_cubicbs(1,t))*(beta+t(random.effects(lme.data)[rs.id,])))+
                alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,t,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,])))))))
  ...
  out=list(beta=beta,sigma2=sigma2,D=D,gamma=gamma,alpha1=alpha1,alpha2=alpha2,cumbase=cumbase,coxts=coxts,sig2=sig2)
}

###calculate log likelihood 

logLik=function(data,data.id,gamma,alpha1,alpha2,beta,sigma2,D,cumbase,knots,Q.2,sig2,L){
  ...
  des.T=cum_cubicbs(1,data.id$Time[data.id$delta==1])
  Time=data.id$Time
  delta=data.id$delta
  W=data.id$W
  K.2=array(c(sapply(data.id$Time[data.id$delta==1],function(t) Q.2%*%cumwei_R.2(1,t,sig2)%*%t(Q.2))),
...
est.EM=function(loglik,data,data.id,gamma,alpha1,alpha2,cumbase,beta,sigma2,D,knots,Q.2,sig2){
  lambda0=function(t){
    if(t %in% unique(data$Time[data$delta==1])){
      x=cumbase[which(near(cumbase[,2],t)),1]-cumbase[which(near(cumbase[,2],t))-1,1] ##warn: options(digits = 22) default 7
      if(length(x)>1){
  ...
  for (k in 1:K){
    ...
    for(v in 0:10){
      ...
      logLikmcnew=logLik(data,data.id,gammanew,alpha1new,alpha2new,betanew,sigma2new,Dnew,cumbase,knots,Q.2,sig2,L=2000)
...
est.sig2=function(data,data.id,gamma,alpha1,alpha2,cumbase,beta,sigma2,D,knots,Q.2,sig2){
  lambda0=function(t){
    if(t %in% unique(data$Time[data$delta==1])){
      x=cumbase[which(near(cumbase[,2],t)),1]-cumbase[which(near(cumbase[,2],t))-1,1] ##warn: options(digits = 22) default 7
      if(length(x)>1){
  ...
  des.T=cum_cubicbs(1,data.id$Time[data.id$delta==1])
  conlogliknew_sig2=function(sig2){
  ...
  conloglik=conlogliknew_sig2(sig2)
  sig2_opt=optim(c(sig2),conlogliknew_sig2,lower=0.01,upper=15,method = "L-BFGS-B")
  
  conloglik_up=sig2_opt$value
  if(conloglik_up<conloglik){
    sig2=sig2_opt$par
    if(abs(conloglik-conloglik_up)/conloglik<10^(-6)){
      flag=1
      return(list(flag=flag,sig2=sig2_opt$par))
    }else{
      return(list(flag=flag,sig2=sig2_opt$par))
    }
  }else{
    flag=1
    return(list(flag=flag,sig2=sig2))
