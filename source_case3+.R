...
########### CONTOUR PLOT of log E^{bi|Yi}(p(Ti|bi)|)
...
#                           alpha2*sqrt(t(beta+bi)%*%K.2[,,match(Time[data.id$id==i],data.id$Time[data.id$delta==1])]%*%(beta+bi)))
#       if(length(a)>1){
#         weinew=c(exp(c(gamma*W[data.id$id==i]))*sapply(a,lambda0)*exp(alpha1*(des.T[match(a,data.id$Time[data.id$delta==1]),]%*%(beta+bi))+
#                                                                         alpha2*apply(K.2[,,match(a,data.id$Time[data.id$delta==1])],3,function(y) sqrt(t(beta+bi)%*%y%*%(beta+bi)))))
#         
#         ZKnew=alpha1*des.T[match(a,data.id$Time[data.id$delta==1]),]+alpha2*apply(K.2[,,match(a,data.id$Time[data.id$delta==1])],3,function(y) (c(t(beta+bi)%*%y%*%(beta+bi)))^(-1/2))*
#           t(apply(K.2[,,match(a,data.id$Time[data.id$delta==1])],3,function(y) t(y)%*%beta)+apply(K.2[,,match(a,data.id$Time[data.id$delta==1])],3,function(y) y%*%bi))
...
#       Sbi=alpha1*des.T[match(Time[i],data.id$Time[data.id$delta==1]),]+alpha2*c(t(beta+bi)%*%K.2[,,match(Time[i],data.id$Time[data.id$delta==1])]%*%(beta+bi))^(-1/2)*K.2[,,match(Time[i],data.id$Time[data.id$delta==1])]%*%(beta+bi)+Sbi
#       
#       Fishinew=-alpha2*(-(c(t(beta+bi)%*%K.2[,,match(Time[data.id$id==i],data.id$Time[data.id$delta==1])]%*%(beta+bi)))^(-3/2)*tcrossprod(t(K.2[,,match(Time[data.id$id==i],data.id$Time[data.id$delta==1])])%*%beta+K.2[,,match(Time[data.id$id==i],data.id$Time[data.id$delta==1])]%*%bi)+
#                           +(c(t(beta+bi)%*%K.2[,,match(Time[data.id$id==i],data.id$Time[data.id$delta==1])]%*%(beta+bi)))^(-1/2)*K.2[,,match(Time[data.id$id==i],data.id$Time[data.id$delta==1])])+Fishinew
#     }
#     
#     Q.fun.up.new=Q.fun.up.new+log(exp(log.hazard-log.s)+sum(diag(inv.Fishi%*%(exp(log.hazard-log.s)*(-Fishinew+tcrossprod(Sbi)))))/2)
#     
#   }
#   return(Q.fun.up.new)
# }
# 
# conlogliknew=function(alpha1,alpha2){
# 
#   conloglik=0
#   for(i in data.id$id[data.id$delta==1]){
#     Ti=data.id$Time[data.id$id==i]
...
#     Conloglik[i,j]=conlogliknew(x[i],y[j])
#   }
# }
# 
