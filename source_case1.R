  ...
  gg_conloglik_3=function(par){
    ...
    for(i in data.id$id[data.id$delta==1]){
      ...
      s2=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(1,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,])))+
                   alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*colSums(c(cumwei_cubicbs(1,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,]))))
      
      i2=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(1,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,])))+
                   alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*
               (colSums(c(cumwei_cubicbs(1,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,]))))^2)
      ...
      i3=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(1,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,])))+
                   alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*
...
est.sig12=function(data,data.id,gamma,alpha1,alpha2,cumbase,beta,sigma2,D,knots,Q.2,sig1,sig2){
  ...
  conlogliknew_sig12=function(sig12){
    ...
    for(i in data.id$id[data.id$delta==1]){
      Ti=data.id$Time[data.id$id==i]
      rs.id=data.id$id[data.id$Time>=Ti]
      conloglik=conloglik+gamma*data.id$W[i]+alpha1*sum(c(cumwei_cubicbs(1,Ti,sig1))*(beta+(bnew_set[i,])))+
        alpha2*sqrt(sum((beta+(bnew_set[i,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+(bnew_set[i,])))))-
        log(sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(1,Ti,sig1))*(beta+t(bnew_set[rs.id,])))+
                      alpha2*sqrt(colSums((beta+t(bnew_set[rs.id,]))*(Q.2%*%cumwei_R.2(1,Ti,sig2)%*%t(Q.2)%*%(beta+t(bnew_set[rs.id,]))))))))
    }
    return(-conloglik)
