...
# mylinbs=function(s){
#   mylinbs1_t=function(s){
#     return(sapply(1:(length(internal_knots)+3+1+2),b_sp,s=s,l=2))
#   }
#   return(t(sapply(s,mylinbs1_t)))
# }
# 
# 
# plot(x=0,y=0,xlim=c(0,10),ylim=c(-4,6),type="n")
# 
# for(t in seq(0,10,0.01)){
#   points(x=t,y=abs(betatrue%*%Q.2%*%(mylinbs(t)[3:7])),col=6)
# }
# 
# for(t in seq(0,10,0.01)){
#   points(x=t,y=betatrue%*%t(mycubicbs(t)),col=1)
# }
...
gendat=function(s,obstim,obsmax,gammatrue,alpha1true,alpha2true,betatrue,D0,sigmatrue,knots,Q.2,sig1true,sig2true){
  ...
  desmat=cumwei_cubicbs(0,seq(10/2,obsmax,by=0.005),sig1true)# design matrix  
  #set.seed(1)
  TD.fun=function(i,t){# i is a scale, t can be a vector
    tt=function(t){
      c(basetrue(t)*exp(gammatrue*W[i]+alpha1true*desmat[match(t,seq(10/2,obsmax,by=0.005)),]%*%(betatrue+b[i,])+
                          alpha2true*sqrt(t(betatrue+b[i,])%*%Q.2%*%cumwei_R.2(0,t,sig2true)%*%t(Q.2)%*%(betatrue+b[i,]))))
    }
    #hazard=sapply(t,tt)
    #p=1-exp(-hazard*0.01)
...
inival=function(data,data.id,ctl,knots,Q.2,low,up){
  ...
  conloglik_3=function(par){
    ...
    for(i in data.id$id[data.id$delta==1]){
      ...
      conloglik=conloglik+gamma*data.id$W[i]+alpha1*sum(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(random.effects(lme.data)[i,])))+
        alpha2*sqrt(sum((beta+t(random.effects(lme.data)[i,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[i,])))))-
        log(sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,])))+
            alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))))
    }
    return(-conloglik)
  ...
  gg_conloglik_3=function(par){
    ...
    for(i in data.id$id[data.id$delta==1]){
      Ti=data.id$Time[data.id$id==i]
      rs.id=data.id$id[data.id$Time>=Ti]
      s=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,])))+
                alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,])))))))
      s1=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,])))+
                  alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*data.id$W[rs.id])
      
      s2=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,])))+
                   alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,]))))
      
      i2=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,])))+
                   alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*
               (colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,]))))^2)
        
      s3=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,])))+
                   alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))
      
      i3=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(random.effects(lme.data)[rs.id,])))+
                   alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))*
               (colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,]))))))
      gg_gamma=gg_gamma+data.id$W[i]-s1/s
      gg_alpha1=gg_alpha1+sum(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(random.effects(lme.data)[i,])))-s2/s
      i_alpha1=i_alpha1+i2/s-(s2/s)^2
      gg_alpha2=gg_alpha2+sqrt(sum((beta+t(random.effects(lme.data)[i,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[i,])))))-s3/s
  ...
  conloglik_sig12=function(sig12){
    sig1=sig12[1]
    sig2=sig12[2]
    conloglik=0
    for(i in data.id$id[data.id$delta==1]){
  ...
  for(k in 1:10){
    ...
    alpha1=res$par[2]
    alpha2=res$par[3]
    sig12=optim(c(sig1,sig2),conloglik_sig12,lower=c(0.01,0.01),upper = c(10,10),method = "L-BFGS-B")
    sig1=sig12$par[1]
    sig2=sig12$par[2]
    if(res$value<sig12$value){
      break
    }else{
      if(abs(res$value-sig12$value)/res$value<10^(-6)){
        break
      }
    }
  }
  
  coxts=c(gamma,alpha1,alpha2,sig1,sig2)
  ...
  for(t in sort(unique(data$Time[data$delta==1]))){
    rs.id=riskset(t)
    s=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,t,sig1))*(beta+t(random.effects(lme.data)[rs.id,])))+
                alpha2*sqrt(colSums((beta+t(random.effects(lme.data)[rs.id,]))*(Q.2%*%cumwei_R.2(0,t,sig2)%*%t(Q.2)%*%(beta+t(random.effects(lme.data)[rs.id,])))))))
  ...
  out=list(beta=beta,sigma2=sigma2,D=D,gamma=gamma,alpha1=alpha1,alpha2=alpha2,cumbase=cumbase,coxts=coxts,sig1=sig1,sig2=sig2)
  
  ################# IS CONDITIONAL LOG LIKELIHOOD EQUAL TO ....?##### YES! #################
  # Time=data.id$Time
  # delta=data.id$delta
  # W=data.id$W
  # des.T=cumwei_cubicbs(0,data.id$Time[data.id$delta==1],sig1)
  # K.2=array(c(sapply(data.id$Time[data.id$delta==1],function(t) Q.2%*%cumwei_R.2(0,t,sig2)%*%t(Q.2))),
  #           dim=c(q,q,length(data.id$Time[data.id$delta==1]))) 
  

  # Q.fun=0
  # for(i in data.id$id){
  #   bi=as.numeric(random.effects(lme.data)[data.id$id==i,])
  #   a=unique(data.id$Time[(data.id$Time<=data.id$Time[data.id$id==i])&(data.id$delta==1)])
  #   log.hazard=0
  #   log.s=0
  #   if(length(a)>0){
  #     log.hazard=ifelse(data.id$delta[data.id$id==i]==0,0,log(lambda0(data.id$Time[data.id$id==i]))+c(gamma*W[data.id$id==i])+alpha1*(des.T[match(Time[data.id$id==i],data.id$Time[data.id$delta==1]),]%*%(beta+bi))+
  ...
  #     }
  #   }
  #   Q.fun=Q.fun+(log.hazard-log.s)
  # }
  # print(Q.fun)
  # 
  # ss=0
  # for(i in data.id$id[delta==1]){
  #   t=Time[i]
  #   ss=ss+log(length(data.id$id[(data.id$Time==t)&(data.id$delta==1)]))
  # }
  # 
  # -res$value+ss-sum(delta)

  
  ##CONTOUR PLOT
  # conloglik=function(alpha1,alpha2){
  #   conloglik=0
  #   for(i in data.id$id[data.id$delta==1]){
  #     Ti=data.id$Time[data.id$id==i]
  #     rs.id=data.id$id[data.id$Time>=Ti]
  ...
  # x=seq(0.2,0.4,by=0.01)
  # y=seq(0.3,0.7,by=0.01)
  # Conloglik=matrix(0,nrow = length(x),ncol=length(y))
  # for(i in c(1:length(x))){
  #   for(j in c(1:length(y))){
  #     Conloglik[i,j]=conloglik(x[i],y[j])
  #   }
  # }
 
  # 
  # contour(x,y,Conloglik,xlab="alpha1",ylab="alpha2",sub="(sig1,sig2) fixed as (2,2),seed=4,bi=E(bi|Y)",main = "contour plot of initial (Two-stage) conditional log likelihood")
  # points(x=0.3,y=0.5,col=2,pch=19)
  
  # 
  # conloglik_true=function(sig1,sig2){
  #   conloglik=0
  #   for(i in data.id$id[data.id$delta==1]){
  #     Ti=data.id$Time[data.id$id==i]
  #     rs.id=data.id$id[data.id$Time>=Ti]
  #     conloglik=conloglik+gammatrue*data.id$W[i]+alpha1true*sum(c(cumwei_cubicbs(0,Ti,sig1))*(betatrue+b[i,]))+
  #       alpha2true*sqrt(sum((betatrue+b[i,])*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(betatrue+b[i,]))))-
  #       log(sum(exp(gammatrue*data.id$W[rs.id]+alpha1true*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(betatrue+t(b[rs.id,])))+
  #                     alpha2true*sqrt(colSums((betatrue+t(b[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(betatrue+t(b[rs.id,]))))))))
  #   }
  #   return(conloglik)
  # 
  # }
  ...
  # for(i in c(1:length(x))){
  #   for(j in c(1:length(y))){
  #     Conloglik[i,j]=conloglik_true(x[i],y[j])
  #   }
  # }
  # 
  # contour(x,y,Conloglik,xlab="sig1",ylab="sig2")
  # points(x=1.9,y=2.1,col=2,pch=19)
  ...
  # logTS.sig12=function(sig12){
  #   sig1=sig12[1]
  #   sig2=sig12[2]
  #   data.ts$preY=numeric(dim(data)[1])
  #   data.ts$preY[data.ts$time>0]=rowSums(cumwei_cubicbs(0,data.ts$time[data.ts$time>0],sig1)*t(beta+t(raneff[data.ts$time>0,])))
  #   data.ts$prevar=numeric(dim(data)[1])
  #   data.ts$prevar[data.ts$time>0]=sapply(which(data.ts$time>0),function(x) sqrt(t(as.numeric(beta+raneff[x,]))%*%(Q.2%*%cumwei_R.2(0,data$time[x],sig2)%*%t(Q.2))%*%(as.numeric(beta+raneff[x,]))))
  #   coxts.data=coxph(Surv(start, stop, event)~W+preY+prevar,data=data.ts)
  #   return(-coxts.data$loglik[2])
  # 
  # }
  # des.sig12=xDRDN(lhsDesign(25,2),dgts = 2, range = c(low, up))
  # logTS.des=apply(des.sig12,1,logTS.sig12)
  # 
  # ##BAYESIAN OPTIMIZATION
  # model=km(design = des.sig12,response = data.frame(logTS.des),
  #          control=list(trace=FALSE))
  # 
  # res=noisy.optimizer(optim.crit = "AKG",model = model,n.ite = 30,noise.var =0,
  #                     funnoise = logTS.sig12,lower=rep(low,2),upper=rep(up,2),NoiseReEstimate = FALSE,CovReEstimate = TRUE,control = list(max.generations=20,wait.generations=10))
  # 
  # sig1=res$best.x[1]
  # sig2=res$best.x[2]
  # 
  ...
  # gamma=coxts.data$coefficients[1]
  # alpha1=coxts.data$coefficients[2]
  # alpha2=coxts.data$coefficients[3]
  # ## TAKE CARE
  # cumbase=rbind(c(0,0),basehaz(coxts.data, centered=FALSE))
  ##plots
  # plot(x=data.ts$time[data.ts$id==1000],y=lme.data$fitted[data.ts$id==1000,2],col=1,type="b",ylim=c(-1,10.5))
  # points(x=data.ts$time[data.ts$id==1000][-1],y=data.ts$prevar[data.ts$id==1000][-1],col=1,type="b",pch=16)
  # points(x=data.ts$time[data.ts$id==993],y=lme.data$fitted[data.ts$id==993,2],col=4,type="b")
  # points(x=data.ts$time[data.ts$id==993][-1],y=data.ts$prevar[data.ts$id==993][-1],col=4,type="b",pch=16)
...
logLik=function(data,data.id,gamma,alpha1,alpha2,beta,sigma2,D,cumbase,knots,Q.2,sig1,sig2,L){
...
est.EM=function(loglik,data,data.id,gamma,alpha1,alpha2,cumbase,beta,sigma2,D,knots,Q.2,sig1,sig2){
  lambda0=function(t){
    if(t %in% unique(data$Time[data$delta==1])){
      x=cumbase[which(near(cumbase[,2],t)),1]-cumbase[which(near(cumbase[,2],t))-1,1] ##warn: options(digits = 22) default 7
      if(length(x)>1){
  ...
  for (k in 1:K){
    ...
    logLikmcnew=logLik(data,data.id,gammanew,alpha1new,alpha2new,betanew,sigma2new,Dnew,cumbase,knots,Q.2,sig1,sig2,L=2000)
    
    cat("UPDATED LOGLIKELIHOOD=",logLikmcnew,"\n")
    #if((Q.fun.up.new>Q.fun)&(logLikmcnew>logLikmc))
    ...
    if(((abs(logLikmcnew-loglik)/abs(loglik))<2*10^(-5))){
      if(logLikmcnew>loglik){
...
##########UPDADE  SIG1 AND SIG2 BY OPTIMIZING PARTIAL LIKELIHOOD
est.sig12=function(data,data.id,gamma,alpha1,alpha2,cumbase,beta,sigma2,D,knots,Q.2,sig1,sig2){
  flag=0
  des.Y=as.matrix(data[,c(paste0("time",c(1:q)))])
  ...
  ################
  conlogliknew_sig12=function(sig12){
    sig1=sig12[1]
    sig2=sig12[2]
    conloglik=0
    for(i in data.id$id[data.id$delta==1]){
      Ti=data.id$Time[data.id$id==i]
      rs.id=data.id$id[data.id$Time>=Ti]
      conloglik=conloglik+gamma*data.id$W[i]+alpha1*sum(c(cumwei_cubicbs(0,Ti,sig1))*(beta+(bnew_set[i,])))+
        alpha2*sqrt(sum((beta+(bnew_set[i,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+(bnew_set[i,])))))-
        log(sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(bnew_set[rs.id,])))+
                      alpha2*sqrt(colSums((beta+t(bnew_set[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(bnew_set[rs.id,]))))))))
    }
    return(-conloglik)
    
  }
  
  conloglik=conlogliknew_sig12(c(sig1,sig2))
  sig12=optim(c(sig1,sig2),conlogliknew_sig12,lower=c(0.01,0.01),upper = c(10,10),method = "L-BFGS-B")
  conloglik_up=sig12$value
  if(conloglik_up<conloglik){
    sig1=sig12$par[1]
    sig2=sig12$par[2]
    if(abs(conloglik-conloglik_up)/conloglik<10^(-6)){
      flag=1
      return(list(flag=flag,sig1=sig12$par[1],sig2=sig12$par[2]))
    }else{
      return(list(flag=flag,sig1=sig12$par[1],sig2=sig12$par[1]))
    }
  }else{
    flag=1
    return(list(flag=flag,sig1=sig1,sig2=sig2))
    
  }

}

# conlogliknew=function(par){
#   gamma=par[1]
#   alpha1=par[2]
#   alpha2=par[3]
#   
...
#     Ti=data.id$Time[data.id$id==i]
#     rs.id=data.id$id[data.id$Time>=Ti]
#     s=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(bnew_set[rs.id,])))+
#                 alpha2*sqrt(colSums((beta+t(bnew_set[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(bnew_set[rs.id,])))))))
#     s1=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(bnew_set[rs.id,])))+
#                  alpha2*sqrt(colSums((beta+t(bnew_set[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(bnew_set[rs.id,]))))))*data.id$W[rs.id])
# 
#     s2=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(bnew_set[rs.id,])))+
#                  alpha2*sqrt(colSums((beta+t(bnew_set[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(bnew_set[rs.id,]))))))*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(bnew_set[rs.id,]))))
# 
#     s3=sum(exp(gamma*data.id$W[rs.id]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(bnew_set[rs.id,])))+
#                  alpha2*sqrt(colSums((beta+t(bnew_set[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(bnew_set[rs.id,]))))))*sqrt(colSums((beta+t(bnew_set[rs.id,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(bnew_set[rs.id,]))))))
#     gg_gamma=gg_gamma+data.id$W[i]-s1/s
#     gg_alpha1=gg_alpha1+sum(c(cumwei_cubicbs(0,Ti,sig1))*(beta+(bnew_set[i,])))-s2/s
#     gg_alpha2=gg_alpha2+sqrt(sum((beta+(bnew_set[i,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+(bnew_set[i,])))))-s3/s
#     
# 
#   }
#   return(c(-gg_gamma,-gg_alpha1,-gg_alpha2))
...
# gg_alpha2=numeric(length(data.id$id[data.id$delta==1]))
# ggg_alpha2=numeric(length(data.id$id[data.id$delta==1]))
# 
# ssalpha2=numeric(length(data.id$id[data.id$delta==1]))
# 
# for(i in data.id$id[data.id$delta==1]){
#   Ti=data.id$Time[data.id$id==i]
# 
...
#   K2=K.2[,,match(Time[i],data.id$Time[data.id$delta==1])]
#   desT=des.T[match(Time[i],data.id$Time[data.id$delta==1]),]
#   s=sum(exp(gamma*data.id$W[rs]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(bnew_set[rs,])))+
#               alpha2*sqrt(colSums((beta+t(bnew_set[rs,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(bnew_set[rs,])))))))
#   
#   ss=sum(exp(gamma*data.id$W[rs]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(b_set[rs,])))+
#               alpha2*sqrt(colSums((beta+t(b_set[rs,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(b_set[rs,])))))))
#   s3=sum(exp(gamma*data.id$W[rs]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(bnew_set[rs,])))+
#                alpha2*sqrt(colSums((beta+t(bnew_set[rs,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(bnew_set[rs,]))))))*sqrt(colSums((beta+t(bnew_set[rs,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(bnew_set[rs,]))))))
#   
#   ss3=sum(exp(gamma*data.id$W[rs]+alpha1*colSums(c(cumwei_cubicbs(0,Ti,sig1))*(beta+t(b_set[rs,])))+
#                alpha2*sqrt(colSums((beta+t(b_set[rs,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(b_set[rs,]))))))*sqrt(colSums((beta+t(b_set[rs,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+t(b_set[rs,]))))))
#   gg_alpha2[match(i,data.id$id[data.id$delta==1])]=sqrt(sum((beta+(bnew_set[i,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+(bnew_set[i,])))))-s3/s
#   ggg_alpha2[match(i,data.id$id[data.id$delta==1])]=sqrt(sum((beta+(b_set[i,]))*(Q.2%*%cumwei_R.2(0,Ti,sig2)%*%t(Q.2)%*%(beta+(b_set[i,])))))-ss3/ss
#   
#   for(j in rs){
#     f=c(exp(alpha1*desT%*%b_set[j,]+
#               alpha2*sqrt(t(beta+b_set[j,])%*%K2%*%(beta+b_set[j,]))))
...
#   Deno=c(t(d)%*%Exp.f)
#   ### score of alpha2; Fisher of alpha2
#   ssalpha2[match(i,data.id$id[data.id$delta==1])]=sqrbetaKb[which(rs==i)]-t(d)%*%Exp.sqrfbetaKb/Deno
# }

# res=optim(c(gamma,alpha1,alpha2),conlogliknew,gg_conlogliknew,method = "BFGS")


## ESTIMATE SIG1 AND SIG2 BY BAYESIAN OPTIMIZATION

# est.sig12=function(data,data.id,gamma,alpha1,alpha2,cumbase,beta,sigma2,D,knots,Q.2,sig1,sig2,loglik){
#   loglik.sig12=function(sig12){
#     sig1=sig12[1]
#     sig2=sig12[2]
#     -logLik(data,data.id,gamma,alpha1,alpha2,beta,sigma2,D,cumbase,knots,Q.2,sig1,sig2,L=100)
#   }
#   
#   des.sig12=xDRDN(lhsDesign(20,2,seed = 1),dgts = 2, range = c(1, 3))
#   des.sig12=rbind(des.sig12,c(sig1,sig2))
#   loglik.des=apply(des.sig12,1,loglik.sig12)
#   #loglik.des=c(loglik.des,loglik)
#   model=km(design = des.sig12,response = data.frame(loglik.des),
#            noise.var=rep(1,dim(des.sig12)[1]),control=list(trace=FALSE))
#   
#   res=noisy.optimizer(optim.crit = "AKG",model = model,n.ite = 20,noise.var =1,
#                       funnoise = loglik.sig12,lower=rep(1,2),upper=rep(3,2),NoiseReEstimate = TRUE,CovReEstimate = TRUE,control = list(max.generations=20,wait.generations=10))
#   # modelnew=res$model
#   # lower=c(res$best.x[1]-0.5,res$best.x[2]-0.5)
#   # upper=c(res$best.x[1]+0.5,res$best.x[2]+0.5)
#   # resnew=noisy.optimizer(optim.crit = "AKG",model = modelnew,n.ite = 20,noise.var =1,
#   #                     funnoise = loglik.sig12,lower=lower,upper=upper,NoiseReEstimate = FALSE,CovReEstimate = TRUE,control = list(max.generations=20,wait.generations=10))
#   
#   return(list(sig1=res$best.x[1],sig2=res$best.x[2]))
# 
# }
