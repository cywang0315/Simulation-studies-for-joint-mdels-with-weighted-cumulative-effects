...
inival=function(data,data.id,ctl,knots,Q.2,low,up){
  ...
  out=list(iter=k,beta=beta,sigma2=sigma2,D=D,gamma=gamma,alpha1=alpha1,alpha2=alpha2,cumbase=cumbase,coxts=coxts,sig1=sig1,sig2=sig2)
  ...
  for (k in 1:K){
    ...
    for(v in 0:10){
      ...
      for(t in sort(unique(data$Time[data$delta==1]))){
        ...
        cc_add=length(data.id$id[(data.id$Time==t)&(data.id$delta==1)])/(c(t(exp(gammanew*W[rs]+alpha1new*xbeta))%*%Exp.f))
    ...
    if(k==1){
      ...
      if(logLikmcnew<loglik){
          ...
          for(t in sort(unique(data$Time[data$delta==1]))){
            ...
            cc_add=length(data.id$id[(data.id$Time==t)&(data.id$delta==1)])/(c(t(exp(gamma*W[rs]+alpha1*xbeta))%*%Exp.f))
            if(cc_add<0){
              Exp.f=numeric(length(rs))
              for(j in rs){
                f=c(exp(alpha1*des.T[match(t,data.id$Time[data.id$delta==1]),]%*%b_set[j,]+
